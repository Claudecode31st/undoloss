  ...
  return (
    <div className="flex items-center justify-between mb-6">
      <div>
        ...
        {subtitle && <p className="text-[11px] t-3 mt-0.5">{subtitle}</p>}
      </div>
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 px-3 py-1.5 glass rounded-full">
          <span className="w-2 h-2 rounded-full bg-emerald-400 pulse-dot" />
          <span className="text-xs t-2">Market: {marketStatus}</span>
        </div>
        <span className="text-xs t-3">Last updated: {time}</span>
        {onRefresh && (
          <button
            onClick={onRefresh}
            disabled={refreshing}
            className="p-1.5 glass rounded-lg t-2 hover:text-orange-400 transition-colors disabled:opacity-50"
          >
