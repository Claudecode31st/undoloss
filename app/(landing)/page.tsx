...
/* ── data ─────────────────────────────────────────────────── */
const features = [
  { icon: DollarSign, title: 'Live PnL Tracking', desc: 'Real-time unrealized P/L, exact breakeven levels, and weighted average entry prices per position.', color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/20' },
  { icon: TrendingUp, title: 'DCA Recovery Engine', desc: 'Stage-by-stage averaging suggestions triggered only when risk levels are acceptable — not reckless.', color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
  { icon: Shield, title: 'Delta-Neutral Hedging', desc: 'Simulate 25%, 50%, or 100% hedge ratios to reduce directional exposure and stabilise portfolio value.', color: 'text-indigo-400', bg: 'bg-indigo-500/10 border-indigo-500/20' },
  { icon: BarChart3, title: 'Risk Score (0–100)', desc: 'Portfolio-wide risk scoring based on drawdown depth, asset concentration, and position diversification.', color: 'text-orange-400', bg: 'bg-orange-500/10 border-orange-500/20' },
  { icon: Zap, title: 'Scenario Simulator', desc: 'Model bull, sideways, and bear market outcomes. See your recovery timeline and required price move per scenario.', color: 'text-yellow-400', bg: 'bg-yellow-500/10 border-yellow-500/20' },
  { icon: Brain, title: 'Behavioural Guard', desc: 'Detects signs of panic selling, overtrading, and aggressive averaging — and warns you before you act.', color: 'text-pink-400', bg: 'bg-pink-500/10 border-pink-500/20' },
];

const strategies = [
  { icon: TrendingUp, name: 'DCA Recovery', desc: 'Gradual averaging at staged levels', color: 'text-emerald-400', border: 'border-emerald-500/20', bg: 'bg-emerald-500/5' },
  { icon: Target, name: 'Break-even Planning', desc: 'Calculate exact recovery price targets', color: 'text-blue-400', border: 'border-blue-500/20', bg: 'bg-blue-500/5' },
  { icon: Shield, name: 'Risk Reduction', desc: 'Trim overexposed positions systematically', color: 'text-orange-400', border: 'border-orange-500/20', bg: 'bg-orange-500/5' },
  { icon: TrendingDown, name: 'Swing Re-entry', desc: 'Sell high, rebuy lower for better basis', color: 'text-purple-400', border: 'border-purple-500/20', bg: 'bg-purple-500/5' },
  { icon: Minus, name: 'Delta-Neutral', desc: 'Hedge to near-zero directional exposure', color: 'text-indigo-400', border: 'border-indigo-500/20', bg: 'bg-indigo-500/5' },
];

const steps = [
  { n: '01', title: 'Add your positions', desc: 'Enter your crypto holdings, entry prices, and amounts. Prices auto-fetch from CoinGecko.' },
  { n: '02', title: 'Pick a strategy', desc: 'Choose a recovery approach — DCA, hedging, risk reduction — matched to your risk tolerance.' },
  { n: '03', title: 'Follow the plan', desc: 'Get a structured action plan with the next 3 logical moves. No guesswork. Just math.' },
];
    ...
    <div className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-black/80 backdrop-blur-xl border-b border-zinc-800/60' : ''}`}>
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link href="/dashboard"
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white text-black text-sm font-semibold hover:bg-zinc-100 transition-all duration-200">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-16 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-16">
            <Link href="/dashboard"
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-7 py-3.5 rounded-xl bg-white text-black font-bold text-sm hover:bg-zinc-100 transition-all duration-200 shadow-[0_0_30px_rgba(255,255,255,0.12)]">
              Launch App Free <ArrowRight size={15} />
            </Link>
            <a href="#how-it-works"
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-7 py-3.5 rounded-xl border border-zinc-700/60 text-zinc-300 font-medium text-sm hover:border-zinc-500 hover:text-white transition-all duration-200">
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-zinc-500 max-w-xl mx-auto">
              Built on pure math, not emotions. Every feature is designed to help you make structured decisions, not impulsive ones.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {features.map(({ icon: Icon, title, desc, color, bg }) => (
              <div key={title} className="group p-6 rounded-2xl border border-zinc-800/50 bg-zinc-900/30 hover:bg-zinc-900/60 hover:border-zinc-700/60 transition-all duration-200">
      ...
      <section id="strategies" className="py-24 px-6 bg-zinc-950/40 border-y border-zinc-800/40">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            <SectionLabel>Strategy Engine</SectionLabel>
            <h2 className="text-3xl md:text-4xl font-black text-white mb-4">
              5 rule-based strategies.<br />
              <span className="text-zinc-500">Zero guesswork.</span>
            </h2>
            <p className="text-zinc-500 max-w-xl mx-auto">
              Each strategy is a structured set of actions. You choose the one that fits your situation — the system does the math.
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            {strategies.map(({ icon: Icon, name, desc, color, border, bg }) => (
              <div key={name} className={`flex flex-col gap-3 p-5 rounded-2xl border ${border} ${bg} hover:scale-[1.02] transition-transform duration-200 cursor-default`}>
            ...
            <div className="relative w-64 h-64">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <svg viewBox="0 0 120 70" className="w-44 h-28">
                  ...
                  <path d="M10 65 A 50 50 0 0 1 110 65" fill="none" stroke="#f97316" strokeWidth="10" strokeLinecap="round"
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h2 className="text-4xl md:text-5xl font-black text-white mb-4">
            ...
            <span style={{
              ...
              WebkitTextFillColor: 'transparent',
              backgroundClip: 'text',
            }}>starts right now.</span>
          </h2>
          <p className="text-zinc-500 mb-8 text-lg">
            No sign-up. No credit card. No data sent to any server.<br />
            Just open the app and start building your structured plan.
          </p>
          <Link href="/dashboard"
            className="inline-flex items-center gap-2 px-8 py-4 rounded-xl bg-white text-black font-bold text-base hover:bg-zinc-100 transition-all duration-200 shadow-[0_0_40px_rgba(255,255,255,0.1)]">
            Launch Undo Loss <ArrowUpRight size={18} />
          </Link>
          <p className="text-xs text-zinc-700 mt-4">This is an educational tool. Not financial advice.</p>
        </div>
      </section>
      ...
      <footer className="border-t border-zinc-800/50 py-10 px-6">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6 text-xs text-zinc-600">
            <Link href="/dashboard" className="hover:text-zinc-400 transition-colors">Launch App</Link>
            <a href="https://github.com/Claudecode31st/undoloss" target="_blank" rel="noopener noreferrer" className="hover:text-zinc-400 transition-colors flex items-center gap-1">
              <ArrowUpRight size={12} /> GitHub
            </a>
            <span>For educational use only. Not financial advice.</span>
          </div>
          <p className="text-[11px] text-zinc-700">© 2025 Undo Loss. All rights reserved.</p>
        </div>
      </footer>
